...
*   **Entendimento e reranking de consulta via LLM (Groq, grátis):** um modelo pequeno classifica a consulta (objeto/pessoa/genérica) e ajusta pesos *só* para esse tipo; depois de fundir os sinais, o top-20 candidato vai pra um segundo modelo Groq **ler a sinopse de verdade** e confirmar quem bate o fato citado (pode ser mais de um candidato), a única etapa do pipeline que julga conteúdo em vez de calcular similaridade. Detalhes: [seção abaixo](#como-o-uso-real-evoluiu-o-algoritmo).
...
`core/query_llm.py` usa a **[Groq](https://console.groq.com)** (camada grátis) para classificar a consulta (`openai/gpt-oss-20b`) e, depois, reler o top-20 da fusão e confirmar quem bate o fato citado (`qwen/qwen3.8-27b`, modelo diferente do de classificação, de propósito: assim as duas etapas não competem pela mesma cota diária de tokens). **Sem `GROQ_API_KEY` no ambiente, o sistema funciona normalmente**: a busca cai só no retrieval por similaridade, sem os dois passos de LLM (a mesma degradação graciosa dos pôsteres sem token TMDB). Nunca cole a chave em código versionado; ela vai em `.env`/`.dvc/config.local`-style, fora do git:
```bash
...
7. **Confirmar mais de um candidato, não só o topo.** Sugestão de quem estava acompanhando: já que o LLM lê todas as sinopses do pool mesmo, por que promover só um? Reordenar os 30 candidatos inteiros foi descartado (formato de saída frágil de validar, e pede pro modelo discriminar entre candidatos onde ele não tem base real nenhuma); a versão adotada pede uma **lista de confirmados** em vez de um único palpite, resolvendo um caso real: para "sonho dentro do sonho", tanto *A Origem* quanto *O Discreto Charme da Burguesia* citam a mesma ideia na sinopse, e o design de um palpite só só conseguia promover um dos dois. Achado no caminho: com `reasoning_effort="low"` o modelo pulava o exame item a item de uma lista de 30 candidatos e respondia pela memória, perdendo *O Discreto Charme* (candidato #19) mesmo com a frase "sonhos dentro de sonhos" literal na sinopse dele; subir para `"medium"` resolvia isso, mas o raciocínio oculto de um modelo de raciocínio custava ~9 mil tokens por chamada contra uma cota Groq grátis de 200 mil tokens por dia (dividida com o entendimento de consulta, que roda em toda busca). Trocar o modelo do reranking para `qwen/qwen3.8-27b` (sem raciocínio oculto) resolveu os dois problemas ao mesmo tempo: examina a lista inteira corretamente e tem cota diária própria, separada da do entendimento de consulta. O pool também caiu de 30 para 20 candidatos por economia, mantendo margem suficiente para cobrir o caso acima (posição #19). Medido nos três splits mais sensíveis a esse tipo de ambiguidade: `hard` nDCG@10 0,473 → 0,654, `object` 0,325 → 0,468, `entity` 0,778 → 0,853, sem regressão em nenhum.
8. **Reagrupar franquia inteira ao redor do #1, sem depender de a LLM adivinhar a sequência certa.** Consulta real: "Nissan Skyline azul e prata arrancada" não subia nenhum filme de Velozes e Furiosos ao #1; o filme certo (o carro é do 2º filme especificamente) ficava na posição #15, e a sinopse da TMDB dele não cita carro, cor nem franquia. Ideia cogitada e descartada: deixar o LLM confirmar por conhecimento próprio, não só pela sinopse fornecida. O problema é que o pool tinha **4 filmes da franquia ao mesmo tempo**; o modelo precisaria saber especificamente qual dos 4 tem aquele carro, não só reconhecer "isso é Velozes e Furiosos", uma aposta bem mais específica e arriscada de acertar do que parece. Resolvido de outra forma: quando o #1 pertence a uma franquia (`belongs_to_collection` da TMDB), os outros filmes dela entram na lista de resultados, mesmo que a fusão os tenha ranqueado longe (`core/inference_client.py::_pull_franchise_siblings`, cache de 3 dias). Não depende de a LLM acertar qual sequência exata; o usuário reconhece visualmente qual é a certa assim que ela aparece na tela.
9. **Essa mudança subiu pra produção validada só com 2 exemplos escolhidos a dedo, sem medir o impacto agregado antes, um engano corrigido no mesmo dia.** Medindo depois nos 5 splits formais (sem custo de Groq, essa etapa não usa LLM): `test` piorou (-0,006), e pior que a média agregada, **toda consulta onde o #1 já estava certo e pertencia a uma franquia** corria risco real de regressão, porque a primeira versão inseria os outros filmes só por ordem de lançamento, sem olhar relevância: "Hobbs e Toreto" tinha *Velozes & Furiosos 7* certo em #3 e caiu pra #7 porque um filme mais antigo da mesma franquia (irrelevante pra aquela consulta) entrou na frente. Dois ajustes, remedidos antes de religar em produção: **(1)** só ativa quando o entendimento de consulta classifica a consulta como objeto ou pessoa; descrição genérica de enredo (a maioria de `dev`/`test`/`hard`) raramente tem a ver com a franquia do #1, e zerar a ativação ali eliminou o risco por completo (medido: 0 disparos, 0 mudança nesses 3 splits); **(2)** filmes que já apareciam nos resultados mantêm a ordem relativa que a fusão/reranking já tinham decidido entre eles; só os que vinham de fora do pool, sem sinal de relevância nenhum, usam ordem de lançamento. Remedido: `entity` nDCG@10 0,778 → 0,780 (zero regressões, antes tinha 3), `object` 0,325 → 0,340 (mediana #13 → #11, melhor que a 1ª tentativa, que só ia a 0,328).
...
- **É aqui que o LLM ajuda de verdade, mas não fecha a lacuna sozinho**: classificar a consulta como "objeto" e dar peso maior ao canal léxico de enredo já ajuda; o reranking por confirmação de lista sobe bem mais que isso, porque esse split tem muito caso de franquia (o mesmo objeto/personagem aparece em várias continuações, ex. "carro DeLorean" bate com os três filmes de *De Volta para o Futuro*, "boneco assassino ruivo" bate com seis filmes do Chucky), e confirmar todos os plausíveis de uma vez é exatamente o que esse split precisa: nDCG@10 0,325 → 0,468, mediana #13 → #5. Ainda não é #1: o gargalo restante é **cobertura de dado** (a Wikipédia às vezes também não cita o objeto), não o algoritmo de fusão ou de reranking.
