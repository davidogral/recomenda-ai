...
- **`object`** (objeto/veículo específico citado, ex. "dodge charger preto"):
  ...
  sinopse (TMDB **e** Wikipédia) às vezes simplesmente não cita o objeto.
...
**Todos** os canais de fusão — BM25, embedding, temático, personagem, enredo —
são *scores de similaridade* (distância vetorial ou sobreposição de termo).
Nenhum lê o candidato e confere se o fato citado está mesmo ali. Isso é uma
categoria de erro que nenhum ajuste de peso ou canal a mais resolve — precisa
de algo que **leia conteúdo e julgue**, não que compare vetores.
...
1. **Entendimento de consulta** (`core/query_llm.py::understand`) — roda
   *antes* da busca. Classifica a consulta em `objeto` / `pessoa` / `genérica`
   ...
   objeto). Sempre ativo quando há chave (`RECOMENDAI_RERANK_LLM` não afeta
   este passo).
2. **Reranking via LLM** (`core/query_llm.py::rerank_pick`) — roda *depois* da
   fusão. Manda o top-30 (candidatos + sinopse **completa**, não a versão
   truncada de 240 caracteres exibida no card) pro Groq, que promove um
...
| Regra | Por quê |
|---|---|
| Reescrita de consulta é **só de acrescentar termo, nunca substitui** o texto original | Substituir/encolher a consulta quebrou casos que já funcionavam — "Mcquen" perdeu pra "Alexandre McQueen" depois de correção ortográfica; uma consulta de 16 palavras reduzida a "bar" passou a casar qualquer título que contivesse a substring |
| Override de peso condicional à consulta (`plot_lexical_weight` maior, `entity_weight=0`) só ativa quando `tipo == "objeto"` | Acrescentar termo tipo "Dodge Charger" virava falso-positivo no canal de personagem (tunado pra nome curto, não filtra "isso não é nome de gente") |
...
| Toda chamada é cacheada por (consulta) ou por (consulta + IDs ordenados dos candidatos) — `data/tmdb_cache/query_llm.json` e `rerank_llm.json` | Groq **não é perfeitamente determinística mesmo com `temperature=0`** (efeito de lote/roteamento na infraestrutura compartilhada) — a mesma consulta repetida 8× deu 3 respostas diferentes antes do cache |
...
- **Reranking com o texto completo da Wikipédia** (em vez da sinopse TMDB) —
  ...
  todos os trechos ≈ 5500–9000+ tokens). Caminho futuro: resumo pré-computado
  offline por filme, não uma chamada de tokens grandes por busca.
...
- **Positivas:** o reranking é a única técnica medida que sobe o split `hard`
  ...
  normal (append-only). Sem `GROQ_API_KEY`, zero risco de deploy — a busca
  volta a ser exatamente o pipeline anterior.
- **Custo/risco:** dependência de uma API grátis de terceiro — 1000
  ...
  precisa ficar de olho em *throttling*. Não-determinismo da Groq é mitigado
  (cache) mas não eliminado — o cache trava na primeira resposta real, que
  pode não ser a "melhor" entre respostas plausíveis.
- **Latência:** sem impacto no caminho sem LLM (idêntico ao pipeline
  anterior). Com LLM, soma 1–2 chamadas de rede por busca — compensado pelo
  cache por consulta/candidatos na prática (a maioria das consultas repete).
