...
# ------------------------------------------------------------------ matrizes
EMBED_CONFIGS: list[dict[str, Any]] = [
    {"name": "e5-large fp32 (ST)", "env": {"RECOMENDAI_EMBED_BACKEND": "st"}, "index_dir": "retrieval/index"},
    {"name": "e5-large INT8 (ONNX)", "env": {"RECOMENDAI_EMBED_BACKEND": "onnx-int8"}, "index_dir": "retrieval/index"},
    {"name": "e5-small fp32 (ST)", "env": {"RECOMENDAI_EMBED_BACKEND": "st"}, "index_dir": "retrieval/index_e5small"},
]
...
def _latency_for(engine, split: str, repeats: int, rerank_pool: int = 0) -> dict:
    ...
    stages: dict[str, list[float]] = {
        k: [] for k in ("encode_cold", "encode_warm", "bm25", "emb_matmul", "kw_matmul", "fuse_argsort", "search_total")
    }
    ...
    for _ in range(repeats):
        for q in qs:
            ...
            dt, _ = _t(lambda: engine._encode(cq))
            ...
            if reranker is not None:
                ...
                dt, _ = _t(
                    lambda: reranker.rerank(
                        q, ids, engine._text_for, retr_scores={t: 0.0 for t in ids}, blend=RERANK_BLEND
                    )
                )
...
def run_worker(kind: str, name: str, device: str) -> None:
    ...
    if kind == "embed":
        ...
        _ = rr.rerank(
            "consulta de teste do benchmark", [eng._movie_ids[0].item()], eng._text_for, retr_scores={}, blend=0.5
        )  # falha alto se .predict quebrar
        rss1 = _rss_mb()
        by_pool = {}
...
def _render_embed(rows: list) -> str:
    ...
    for r in rows:
        ...
        out.append(
            f"| {r['name']} | {r['embed_dim']} | {m['ndcg@10']:.3f} | {m['mrr']:.3f} "
...
def _render_rerank(rows: list) -> str:
    ...
    for r in rows:
        for pool, d in r["by_pool"].items():
            ...
            out.append(
                f"| {r['name']} | {pool} | {m['ndcg@10']:.3f} | {m['mrr']:.3f} "
...
def _write(payload: dict, tag: str) -> None:
    ...
    for path in (os.path.join(RESULTS_DIR, f"{stamp}__{tag}.json"), os.path.join(RESULTS_DIR, f"latest__{tag}.json")):
