...
from eval.pipelines import (NEEDS_EMBEDDINGS, PIPELINE_LABELS, PIPELINES, POOL,
                            make_ctx, rank_of)
...
def _engine_meta() -> dict:
    ...
    try:
        with open(p, encoding="utf-8") as fh:
            m = json.load(fh)
        return {k: m[k] for k in ("n_movies", "embed_model", "embed_dim", "lexical")
                if k in m}
    except Exception:
        return {}
...
def _env_snapshot() -> dict:
    from retrieval.search_engine import (DEFAULT_EMBED_WEIGHT, DEFAULT_KEYWORD_WEIGHT,
                                         DEFAULT_POP_PRIOR, RERANK_BLEND, RERANK_POOL)
    return {
...
def evaluate(split: str, pipeline_keys: list[str], limit: int | None,
             quiet: bool) -> dict:
    ...
    if not quiet:
        print(f"» carregando motor de busca…")
    ...
    for key in pipeline_keys:
        ...
        for q in queries:
            ...
            ranks.append(pos)
            times_ms.append(dt)
            per_query.append({"qid": q.qid, "source": q.source,
                              "title": q.relevant_title, "relevant_id": q.relevant_id,
                              "rank": pos, "rr": round(M.reciprocal_rank(pos), 4)})
        ...
        if not quiet:
            print(f"MRR={agg['mrr']:.3f}  nDCG@10={agg['ndcg@10']:.3f}  "
...
_COLS = [
    ...
    ("recall@10", "R@10"), ("recall@50", "R@50"), ("precision@10", "P@10"),
]
...
def render_table(payload: dict) -> str:
    res = payload["results"]
    lines = [
        f"| Pipeline | " + " | ".join(h for _, h in _COLS) + " | mediana | lat. p50 |",
        "|" + "---|" * (len(_COLS) + 3),
    ]
...
def _write_outputs(payload: dict, out: str | None, quiet: bool) -> None:
    ...
    row = {"timestamp_utc": payload["run"]["timestamp_utc"],
           "git_commit": payload["run"]["git_commit"],
           "split": split, "n_queries": payload["run"]["n_queries"],
    ...
    for key in payload["run"]["pipelines"]:
        if key in payload["results"]:
            ...
            row[key] = {mk: m[mk] for mk in ("mrr", "ndcg@10", "recall@10",
                                             "recall@50", "precision@10", "median_rank")}
...
def sweep_rerank(split: str, pools: list[int], limit: int | None,
                 quiet: bool) -> dict:
    ...
    for pool in pools:
        ...
        if not quiet:
            print(f"nDCG@10={agg['ndcg@10']:.3f}  MRR={agg['mrr']:.3f}  "
                  f"R@50={agg['recall@50']:.3f}  p50={lat['p50_ms']:.0f}ms")

    return {
        "run": {
            "timestamp_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "git_commit": _git_commit(), "mode": "sweep_rerank", "split": split,
            "n_queries": len(queries), "dataset_sha1": dataset_sha1(),
            "pools": pools, "engine": _engine_meta(), "config": _env_snapshot(),
        },
        "sweep": rows,
    }
...
def render_sweep(payload: dict) -> str:
    lines = ["| pool cross-encoder | nDCG@10 | MRR | R@10 | R@50 | mediana | lat. p50 | lat. p90 |",
             "|---|---|---|---|---|---|---|---|"]
    for r in payload["sweep"]:
        m, lat = r["metrics"], r["latency_ms"]
        med = f"#{m['median_rank']}" if m["median_rank"] is not None else "—"
        label = "0 (desligado)" if r["pool"] <= 0 else str(r["pool"])
        lines.append(f"| {label} | {m['ndcg@10']:.3f} | {m['mrr']:.3f} | {m['recall@10']:.3f} "
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument("--fast", action="store_true",
                    help="pula fusion_rerank (não carrega o cross-encoder).")
    ap.add_argument("--sweep-rerank", action="store_true",
                    help="varre o tamanho do pool do cross-encoder (curva qualidade × latência).")
    ap.add_argument("--pools", default="0,10,20,50,100,300",
                    help="tamanhos de pool para --sweep-rerank (0 = desligado).")
    ...
    if args.sweep_rerank:
        ...
        print(f"\n### Varredura do re-ranker — split `{args.split}` "
              f"({payload['run']['n_queries']} consultas)\n")
    ...
    keys = ([k.strip() for k in args.pipelines.split(",")] if args.pipelines
            else list(PIPELINES))
    for k in keys:
    ...
    print(f"\n### Ablação por sinal — split `{args.split}` "
          f"({payload['run']['n_queries']} consultas)\n")
